...
func WithArrayJoinFirst(formerName string) (string, string) {
	...
	return fmt.Sprintf("arrayJoin(JSONExtractArrayRaw(%s)) as %s", formerName, nextName), nextName
}
...
func JsonRouterGenerateSelectSql(f reflect.StructField, subClass *ConfigSubClassStruct) {
	...
	subClass.SelectSqls = append(subClass.SelectSqls, fmt.Sprintf("%s(%s,'%s') as %s", j.JsonExtract, lastName, column, fmt.Sprintf("%s%s", subClass.Prefix, column)))
}
